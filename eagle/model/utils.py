...
def _profile_time(profiler, name):
    if profiler is None:
        return nullcontext()
...
def initialize_tree(input_ids, model, past_key_values, logits_processor, profiler=None):
    with _profile_time(profiler, "prefill"):
        outputs, orig, hidden_states = model(
            input_ids, past_key_values=past_key_values, output_orig=True
        )

    with _profile_time(profiler, "sampling"):
        if logits_processor is not None:
            logits = orig[:, -1]
            logits = logits_processor(None, logits)
            probabilities = torch.nn.functional.softmax(logits, dim=1)
    ...
    draft_tokens, retrieve_indices,tree_mask,tree_position_ids = model.ea_layer.topK_genrate(
        ...
        logits_processor,
        profiler=profiler,
        spec_mode=getattr(model, "spec_mode", "baseline"),
        opt_tree_config=getattr(model, "opt_tree_config", None),
        ddd_config=getattr(model, "ddd_config", None),
    )
...
def update_inference_inputs(
    ...
    with _profile_time(profiler, "accept_update"):
